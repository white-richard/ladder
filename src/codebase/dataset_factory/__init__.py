...
try:
    ...
    class UrbanCars:  # fallback stub so other imports succeed
        def __init__(self, *args, **kwargs):
            raise ModuleNotFoundError(
                "dataset_factory.UrbanCars is missing. "
                "Download it (see README) if you actually need the UrbanCars dataset."
            ) from exc
...
from dataset_factory.datasets import Dataset_NIH, EmbedDataset, RSNADataset, collate_NIH
...
def get_rsna_dataloaders(args):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=8, shuffle=False, num_workers=16, pin_memory=True, drop_last=False
    )
    valid_loader = DataLoader(
        valid_dataset, batch_size=8, shuffle=False, num_workers=16, pin_memory=True, drop_last=False
    )
    test_loader = DataLoader(
        test_dataset, batch_size=8, shuffle=False, num_workers=16, pin_memory=True, drop_last=False
    )
    print("\n ==========================> [Shapes] Dataset <==========================")
...
def create_dataloaders(args):
    if (
        args.dataset.lower() == "waterbirds"
        or args.dataset.lower() == "celeba"
        or args.dataset.lower() == "metashift"
    ):
