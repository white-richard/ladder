...
def discover_error_slices_via_sent_waterbirds(
    ...
    calculate_worst_group_acc_waterbirds(
        df, pred_col=pred_col, attribute_col="attribute_bg_predict", log_file=out_file
    )
...
def config():
    parser = argparse.ArgumentParser(
        description="Discover and explain classifier error slices using text embeddings."
    )
    parser.add_argument(
...
def main(args):
    ...
    if Path(args.save_path).exists():
        for pattern in [
            "*_sent_*_emb.txt",
            "ladder_discover_slices_performance_ERM.txt",
        ]:
