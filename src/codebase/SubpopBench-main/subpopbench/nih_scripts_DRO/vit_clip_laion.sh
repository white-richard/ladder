#!/bin/sh
#SBATCH --output=src/psc_logs/subpopbench/nih-GroupDRO/nih-vit_clip_laion-%j.out

pwd
hostname
date

CURRENT=$(date +"%Y-%m-%d_%T")

echo $CURRENT

slurm_output_train1=src/psc_logs/subpopbench/nih-GroupDRO/nih-vit_clip_laion-$CURRENT.out

echo "Save image reps"
source /ocean/projects/asc170022p/shg121/anaconda3/etc/profile.d/conda.sh

conda activate /restricted/projectnb/batmanlab/shawn24/breast_clip_rtx_6000


python src/codebase/SubpopBench-main/subpopbench/train.py \
       --seed 0 \
       --algorithm "GroupDRO" \
       --dataset "NIH_dataset" \
       --train_attr yes \
       --data_dir "data" \
       --output_dir "out/NIH_Cxrclip" \
       --output_folder_name "vit_clip_laion" \
       --image_arch "vit_clip_laion" \
       --es_metric overall:AUROC --use_es >$slurm_output_train1




