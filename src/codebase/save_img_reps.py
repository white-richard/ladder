...
from utils import get_input_shape, seed_all
...
def config():
    parser = argparse.ArgumentParser(
        description="Generate image embeddings and evaluate performance metrics."
    )
    parser.add_argument(
        "--dataset",
        default="NIH",
    ...
    parser.add_argument(
        "--data_dir", default="./data", type=str, help="Root directory of the dataset."
    )
    parser.add_argument(
        "--classifier",
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        default="",
        help="Path to pretrained CLIP checkpoint (if used).",
    )
    parser.add_argument(
        "--save_path",
        metavar="DIR",
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Tokenizer path required by CXR-CLIP or Mammo-CLIP.",
    )
    parser.add_argument(
        "--cache_dir",
        default="",
...
def compute_performance_metrics(dataset, additional_info, save_path, mode):
    ...
    if dataset.lower() == "waterbirds":
        ...
        print(
            f"targets: {targets.shape}, preds: {preds.shape}, attributes: {attributes.shape}, gs: {gs.shape}"
        )
        ...
        print(
            f"\tmean:\t[{final_results['overall']['accuracy']:.3f}]\n"
        ...
        oof_df_agg = (
            df[["patient_id", "laterality", "out_put_GT", "out_put_predict"]]
        ...
        overall_pneumothorax_acc = (
            pneumothorax_df[pneumothorax_df["Predictions_bin"] == 1].shape[0]
            / pneumothorax_df.shape[0]
        )

        pneumothorax_w_tube_df = df[(df["GT"] == 1) & (df["Tubes"] == 1)]
        pneumothorax_w_tube_acc = (
            pneumothorax_w_tube_df[pneumothorax_w_tube_df["Predictions_bin"] == 1].shape[0]
            / pneumothorax_w_tube_df.shape[0]
        )

        pneumothorax_wo_tube_df = df[(df["GT"] == 1) & (df["Tubes"] == 0)]
        pneumothorax_wo_tube_acc = (
            pneumothorax_wo_tube_df[pneumothorax_wo_tube_df["Predictions_bin"] == 1].shape[0]
        ...
        print(
            f"[Shape] tube: {pneumothorax_w_tube_df.shape[0]}, no tube: {pneumothorax_wo_tube_df.shape[0]}"
        )
...
def init_additional_info(dataset):
    ...
    """
    if (
        dataset.lower() == "waterbirds"
        or dataset.lower() == "celeba"
...
def update_additional_info(additional_info, batch_info, dataset):
    ...
    if (
        ...
        additional_info["gs"].append(
            [f"y={yi},a={gi}" for c, (yi, gi) in enumerate(zip(y, bg_attr))]
        )
        additional_info["idx"].append(idx)
        return additional_info

    elif dataset.lower() == "nih":
        y, y_pred, tube, effusion, img_path = (
            batch_info["y"],
            batch_info["y_pred"],
            batch_info["tube"],
            batch_info["effusion"],
        ...
        y, y_pred, mass, calc, mole, mark, scar, clip, patient_id, laterality = (
            batch_info["y"],
            batch_info["y_pred"],
            batch_info["mass"],
            batch_info["calc"],
        ...
        additional_info["patient_id"] = torch.cat(
            (additional_info["patient_id"], patient_id), dim=0
        )
        additional_info["laterality"] = torch.cat(
            (additional_info["laterality"], laterality), dim=0
        )
        additional_info["out_put_GT"] = torch.cat((additional_info["out_put_GT"], y), dim=0)
        additional_info["out_put_predict"] = torch.cat(
            (additional_info["out_put_predict"], y_pred), dim=0
...
def process_batch(batch, device, clf, clip_model, classifier_type, dataset):
    ...
    if (
        ...
        dataset.lower() == "nih"
        and clip_model["type"] == "cxr_clip"
        and classifier_type.lower() == "resnet50"
    ):
        img, y, tube, effusion, text = (
            batch["img"],
            batch["label"],
            batch["tube"],
            batch["effusion"],
        ...
        reps_clip = (
            clip_model["model"].image_projection(reps_clip)
...
def main(args):
    ...
    args.save_path = (
        Path(args.save_path.format(args.seed)) / f"clip_img_encoder_{args.clip_vision_encoder}"
    )
