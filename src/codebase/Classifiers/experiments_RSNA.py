...
def do_experiments(args, device):
    ...
    for fold in range(args.start_fold, args.n_folds):
        ...
        if dataset_name == "rsna":
            ...
            args.train_folds = args.df[(args.df["fold"] == 1) | (args.df["fold"] == 2)].reset_index(
                drop=True
            )
            ...
            args.train_folds = args.train_folds.rename(
                columns={"ImageLateralityFinal": "laterality"}
            )
            args.valid_folds = args.valid_folds.rename(
                columns={"ImageLateralityFinal": "laterality"}
            )

            args.BCE_weights = {}
            args.BCE_weights[f"fold{args.cur_fold}"] = (
                args.train_folds[args.train_folds[args.label] == 0].shape[0]
    ...
    oof_df_agg_cancer["prediction"] = oof_df_agg_cancer["prediction"].apply(
        lambda x: 1 if x >= 0.5 else 0
    )
    acc_cancer = compute_accuracy_np_array(
        oof_df_agg_cancer[args.label].values, oof_df_agg_cancer["prediction"].values
    )
    ...
    oof_df.to_csv(
        args.output_path / f"seed_{args.seed}_n_folds_{args.n_folds}_outputs.csv", index=False
    )


def train_loop(args, device):
    print(f"\n================== fold: {args.cur_fold} training ======================")
    args.train_folds = args.train_folds.sample(
        frac=args.data_frac, random_state=1, ignore_index=True
    )
    ...
    for epoch in range(args.epochs):
        ...
        valid_agg = None
        dataset_name = normalize_mammo_dataset_name(args.dataset)
        if dataset_name == "vindr" or dataset_name == "cbis":
        ...
        valid_agg_cancer["prediction"] = valid_agg_cancer["prediction"].apply(
            lambda x: 1 if x >= 0.5 else 0
        )
        acc_cancer = compute_accuracy_np_array(
            valid_agg_cancer[args.label].values, valid_agg_cancer["prediction"].values
        )
...
def valid_fn(valid_loader, model, criterion, args, device, epoch=1, attr_embs=None, logger=None):
    ...
    if (
        args.label.lower() == "density"
        or args.label.lower() == "birads"
        or args.label.lower() == "race"
        or args.label.lower() == "age_group"
    ):
