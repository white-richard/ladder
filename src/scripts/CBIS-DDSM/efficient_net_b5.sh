python ./src/codebase/train_classifier_Mammo.py \
  --data-dir "$HOME/.code/datasets/cbis-ddsm" \
  --img-dir "processed_png_full" \
  --csv-file "metadata/manifest_full_mammograms.csv" \
  --dataset "CBIS-DDSM" --arch "tf_efficientnet_b5_ns-detect" --epochs 12 --batch-size 8 --num-workers 0 \
